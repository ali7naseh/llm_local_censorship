#!/usr/bin/env python3
"""
Template for model-level local censorship analysis.

This script illustrates how to compare each target model against a reference
set of models on the same prompts. It assumes that model responses have already
been scored by LLM judges and that the score files contain common prompt IDs
or prompt text.

The original experiments used this structure to compare target models against
a reference set using calibrated judge scores.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run model-level local censorship analysis.")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="JSON file specifying target models and reference models.",
    )
    parser.add_argument(
        "--calib_file",
        type=str,
    ...
    parser.add_argument(
        "--output_file",
        type=str,
        default="outputs/local_results.csv",
        help="Path to save local censorship results.",
    )
    ...
    experiments = load_json(args.experiments_json)
    ...
    # 3. Build score matrix, optionally filtered by category/subcategory.
    ...
    available_models = set(score_matrix.columns)

    rows = []

    # 4. Run each local-censorship experiment.
    for exp in experiments:
        target_model = exp["target_model"]
        reference_models = exp["reference_models"]

        if target_model not in available_models:
            print(f"Skipping {target_model}: target not found in score matrix.")
            continue

        missing_refs = [m for m in reference_models if m not in available_models]
        if missing_refs:
            print(f"Skipping {target_model}: missing reference models {missing_refs}.")
            continue

        res = analyze_local_censorship(
            score_matrix,
        ...
        rows.append({
            "target_model": target_model,
            "reference_models": ", ".join(reference_models),
            ...
            "local_censorship": res["local_censorship"],
            "Category": args.target_category,
            "Sub_Category": args.target_sub_category,
        })
    ...
    print(f"Saved local censorship results to: {args.output_file}")
